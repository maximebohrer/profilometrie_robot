...
- La fonction `GetProfileAdvance(deviceID)` permet de récupérer un seul profil lorsque le profilomètre n'est pas en mode batch. La fonction renvoie un nuage de points de la forme [[x, y, z], [x, y, z], ...] (y étant toujours nul puisqu'on a qu'un seul profil).
...
Le fichier `traitement_face.py` permet aussi de réduire le bruit du nuage de point. Le nuage de point traité est enregistré dans l'arborescence suivante, en format `txt` : `data/nuage_filtered_outliers_removed_30_2.txt`.
